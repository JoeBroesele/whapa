...
#    config = ConfigObj(cfg_file, interpolation=None)
    try:
        ...
        password = settings['gmail_passw']
        celnumbr = settings['celnumbr']
        oauth_token = settings['oauth']
        android_id = settings['android_id']
        if not whautils.check_google_password():
            settings['gmail_passw'] = getpass.getpass(prompt="Google password for account '" + settings['gmail'] + "': ", stream=None)
        if not settings['gmail_passw']:
